...
// because otherwise people can "react" to it by voting in the opposite direction (without sock-puppet accounts).
    ...
    function processPR(pr) {
        ...
        // only make a decision if we have (MIN_VOTES * REQUIRED_SUPERMAJORITY) total 
        // votes either way. 
        var winningTotal = Math.Max(voteResults.positive, voteResults.negative);
        if (winningTotal < MIN_VOTES * REQUIRED_SUPERMAJORITY) { return; }
